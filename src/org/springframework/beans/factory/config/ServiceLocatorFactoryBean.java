 ...
 *}</pre>
 * 
 * <p>See {@link org.springframework.beans.factory.config.ObjectFactoryCreatingFactoryBean} for an alternate approach.
 *
...
public class ServiceLocatorFactoryBean implements FactoryBean, BeanFactoryAware, InitializingBean {
	 ...
	 * the signatures <code>MyType xxx()</code> or <code>MyType xxx(MyIdType id)</code>
	 * (typically, <code>MyService getService()</code> or <code>MyService getService(String id)</code>).
	 * See the {@link org.springframework.beans.factory.config.ServiceLocatorFactoryBean class-level Javadoc} for
	 * information on the semantics of such methods.
	 * @param interfaceType the {@link Class} of the interface to be used for the service locator
	 */
